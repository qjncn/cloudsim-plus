 ...
 *
 */
public class CloudletToVmMappingQlearn(Cloudlet cloudlet)
{
     ...
     * 构造Q矩阵，默认是0
     */
    int NumOfCloudlet = cloudletList.size();
    int NumOfVM = vmList.size();
    ...
    for(int CloudID=0;CloudID<NumOfCloudlet;CloudID++) { // 到达目标状态，结束循环，进行下一轮训练
        ...
        double reward = - graph[CloudID][VmID]; // 奖励，是延时的负数
        //更新Q表
